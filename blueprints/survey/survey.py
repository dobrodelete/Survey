...
def survey_login():
    with open("static/json/iogv.json", "r", encoding="utf8") as file:
        iogv = json.load(file)
...
def survey_instruction():
    ...
    if request.method == "POST":
        resp = make_response(render_template("survey/instruction.html", title="Инструкция"))
        resp.set_cookie("subdivision", request.values.get("subdivision"), max_age=60*60*12)
        resp.set_cookie("post", request.values.get("post"), max_age=60*60*12)
        resp.set_cookie("iogv", request.values.get("iogv"), max_age=60*60*12)
        return resp
